...
- Старые checkpoint, созданные до добавления coverage-канала или изменения числа входных каналов, несовместимы с текущей `BEVSegNet`.
- Значения `depth_scale` и `default_depth` не влияют на основной режим `--no_depth`, потому что используется пересечение лучей с плоскостью земли.
- При `num_workers > 0` включаются persistent workers. Для первого запуска на Kaggle безопасно оставить `num_workers=0`, затем попробовать `2` или `4`.
- `pixel_step=1` дает максимально плотную проекцию, но заметно увеличивает время и память. Для быстрых экспериментов удобно использовать `2`, `4` или `8`.
- Порог `0.5` не обязательно дает лучший IoU. Его полезно подобрать на validation после обучения.
